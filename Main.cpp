...
void draw_hud(const HashTable<Team, int32>& tickets, const Array<CapturePoint>& capture_points,
	...
	for (size_t i = 0; i < capture_points.size(); ++i) {
		...
		int32 x = start_x + (int32)i * 50 + 20; int32 y = 28;
...
void draw_minimap(const Soldier& player, const Array<Soldier>& allies, const Array<Soldier>& enemies,
	...
	double m_size = 140.0; double mx = SCREEN_WIDTH - m_size - 20.0; double my = 75.0;
	// ミニパックの枠線と背景
...
void reset_game(Array<Obstacle>& obstacles, Array<CapturePoint>& capture_points, Soldier& player,
	...
	allies.clear(); for (int i = 0; i < 9; ++i) allies.emplace_back(Random(150, 350), Random(150, 350), Team::Blue);
	enemies.clear(); for (int i = 0; i < 12; ++i) enemies.emplace_back(Random(1650, 1850), Random(1650, 1850), Team::Red);
	bullets.clear(); tickets = { {Team::Blue, 250}, {Team::Red, 250} };
}
...
void Main() {
	...
	Array<Obstacle> obstacles; Array<CapturePoint> capture_points; Soldier player(0, 0, Team::None);
	Array<Soldier> allies; Array<Soldier> enemies; Array<Bullet> bullets; HashTable<Team, int32> tickets;

	// マップ・障害物・拠点の配置、プレイヤー/AIの初期配置、初期チケットの設定
	reset_game(obstacles, capture_points, player, allies, enemies, bullets, tickets);
	double ticket_decay_timer = 0.0; bool game_over = false; Team winner = Team::None;
	...
	// 2. チケット・勝敗判定
		if (!game_over) {
			...
			if (ticket_decay_timer >= 1.0) {
				...
				for (const auto& cp : capture_points) { if (cp.owner == Team::Blue) blue_cps++; else if (cp.owner == Team::Red) red_cps++; }
				if (blue_cps > red_cps) tickets[Team::Red] = Max(0, tickets[Team::Red] - (blue_cps - red_cps));
				else if (red_cps > blue_cps) tickets[Team::Blue] = Max(0, tickets[Team::Blue] - (red_cps - blue_cps));
			}
			// どっちか0以下でゲームオーバー
			if (tickets[Team::Blue] <= 0) { game_over = true; winner = Team::Red; }
			else if (tickets[Team::Red] <= 0) { game_over = true; winner = Team::Blue; }
		}
	// 3. プレイヤーの移動
		if (!player.dead && !game_over) {
			double dx = 0, dy = 0;
			if (KeyA.pressed()) dx -= 240.0 * dt; if (KeyD.pressed()) dx += 240.0 * dt;
			if (KeyW.pressed()) dy -= 240.0 * dt; if (KeyS.pressed()) dy += 240.0 * dt;
			// 障害物、マップ端の衝突判定
	...
	// 4. 拠点の更新
		...
		for (auto& a : allies) all_soldiers.push_back(&a);
		for (auto& e : enemies) all_soldiers.push_back(&e);
		// 各拠点の人数判定、占領進捗更新
		if (!game_over) for (auto& cp : capture_points) cp.update(all_soldiers, dt);

	// 5. AIの更新
		// 味方AI,敵AIそれぞれのポインタ配列作成
		Array<Soldier*> ptr_allies, ptr_enemies;
		for (auto& a : allies) ptr_allies.push_back(&a);
		for (auto& e : enemies) ptr_enemies.push_back(&e);
		...
		for (auto& a : allies) {
			...
			// 生存してれば思考と行動
			if (!a.dead && !game_over) a.think_and_move(&player, ptr_enemies, Array<Soldier*>(), bullets, obstacles, capture_points, dt);
			// 死亡時の処理
			if (a.health <= 0 && !a.dead) { a.dead = true; a.respawn_timer = 3.0; tickets[Team::Blue] = Max(0, tickets[Team::Blue] - 1); }
		}
		// 敵AI更新
		for (auto& e : enemies) {
			// 死亡してればリスポーンタイマーの進捗
			...
			if (!e.dead && !game_over) e.think_and_move(&player, Array<Soldier*>(), ptr_allies, bullets, obstacles, capture_points, dt);
			// 死亡時の処理
			if (e.health <= 0 && !e.dead) { e.dead = true; e.respawn_timer = 3.0; tickets[Team::Red] = Max(0, tickets[Team::Red] - 1); }
		}

		// プレイヤーの死亡・リスポーン処理
		if (player.health <= 0 && !player.dead) { player.dead = true; player.respawn_timer = 3.0; tickets[Team::Blue] = Max(0, tickets[Team::Blue] - 1); }
	...
	// 6. 弾丸の更新&衝突判定
		if (!game_over) {
			for (auto it = bullets.begin(); it != bullets.end();) {
				...
				// マップ外は削除
				if (it->pos.x < 0 || it->pos.x > MAP_WIDTH || it->pos.y < 0 || it->pos.y > MAP_HEIGHT) { it = bullets.erase(it); continue; }

				// 障害物との衝突判定
				bool hit_wall = false;
				for (const auto& wall : obstacles) { if (wall.rect.intersects(it->pos)) { hit_wall = true; break; } }
				if (hit_wall) { it = bullets.erase(it); continue; }

				// 兵士との衝突判定
				bool hit_soldier = false;
				for (auto target : all_soldiers) {
					if (!target->dead && it->team != target->team && it->pos.distanceFrom(target->pos) < 20.0) {
						target->health -= 15.0; hit_soldier = true; break;
					}
				}
				if (hit_soldier) it = bullets.erase(it); else ++it;
			}
		}
	...
	// 7. 画面描画
		...
		for (int32 x = 0; x < MAP_WIDTH; x += 100) {
			...
			if (0 <= line_x && line_x <= SCREEN_WIDTH) Line(line_x, 0, line_x, SCREEN_HEIGHT).draw(COLOR_GRID);
		}
		// グリッド線(縦)の描画：100px間隔
		for (int32 y = 0; y < MAP_HEIGHT; y += 100) {
			double line_y = y - camera.y;
			if (0 <= line_y && line_y <= SCREEN_HEIGHT) Line(0, line_y, SCREEN_WIDTH, line_y).draw(COLOR_GRID);
		}

		// 各種オブジェクトをカメラ位置からの相対座標で描画
		for (const auto& cp : capture_points) cp.draw(camera, fontCPText);
		for (const auto& wall : obstacles) wall.draw(camera);
		player.draw(camera, fontState);
		for (const auto& a : allies) a.draw(camera, fontState);
		for (const auto& e : enemies) e.draw(camera, fontState);
		for (const auto& b : bullets) b.draw(camera);
